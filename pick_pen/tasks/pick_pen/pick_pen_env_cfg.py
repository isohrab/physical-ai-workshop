...
from leisaac.assets.scenes.kitchen import KITCHEN_WITH_PEN_CFG, KITCHEN_WITH_PEN_USD_PATH
...
class PickPenSceneCfg(InteractiveSceneCfg):
    """Scene configuration for the pick pen task."""

    scene: AssetBaseCfg = KITCHEN_WITH_PEN_CFG.replace(prim_path="{ENV_REGEX_NS}/Scene")
...
class ObservationsCfg:
    ...
    class SubtaskCfg(ObsGroup):
        ...
        pick_pen = None  # YOUR CODE HERE: ObsTerm(func=mdp.pen_grasped, params={"object_cfg": SceneEntityCfg("MechanicalPencil")})

        # TODO: Add pen placement observation term
        put_pen_to_plate = None  # YOUR CODE HERE: ObsTerm(func=mdp.put_pen_to_plate, params={"object_cfg": SceneEntityCfg("MechanicalPencil"), "plate_cfg": SceneEntityCfg("Plate")})
...
class PickPenEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the pick pen environment."""

    scene: PickPenSceneCfg = PickPenSceneCfg(env_spacing=8.0)
    ...
    def __post_init__(self) -> None:
        ...
        parse_usd_and_create_subassets(
            KITCHEN_WITH_PEN_USD_PATH, self, specific_name_list=["MechanicalPencil", "Plate"]
        )
        ...
        domain_randomization(
            ...
            random_options=[
                randomize_object_uniform(
                    "MechanicalPencil", pose_range={"x": (-0.013, 0.013), "y": (-0.013, 0.013), "z": (0.0, 0.0)}
                ),
                ...
                randomize_camera_uniform(
                    ...
                    pose_range={
                        ...
                        "y": (-0.025, 0.025),
                        "z": (-0.025, 0.025),
                        "roll": (0 * torch.pi / 180, 0 * torch.pi / 180),
                        "pitch": (0 * torch.pi / 180, 0 * torch.pi / 180),
                        "yaw": (0 * torch.pi / 180, 0 * torch.pi / 180),
                    },
                    convention="ros",
                ),
            ],
